(ns nervestaple.mimeo.log.interface
  (:require
   [nervestaple.mimeo.log.core :as core]))
...
(defmacro with-level
  "Accepts a log level and form to evaluate. Temporarily sets the log level to the
  provided level, evaluates the form, resets the log level and returns the
  form's results."
  [level & args]
  `(core/with-level ~level ~@args))

(defmacro with-ns-level
  "Accepts a namespace, log level and form to evaluate. Temporarily sets the log
  level for the namespace to the provided level, evaluates the form, resets the
  log level and returns the form's results."
  [ns level & args]
  `(core/with-ns-level ~ns ~level ~@args))
